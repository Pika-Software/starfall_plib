...
hook.add('think', chipName, function()
    local ok, ent = pcall( prop.create, plib.GetPlayerCenterPos( plib.Owner ), plib.Owner:getAngles(), 'models/hunter/blocks/cube6x6x6.mdl', true )
    if (ok) then
        ...
        hook.add( 'think', chipName, function()
            ...
            ent:setPos( plib.GetPlayerCenterPos( plib.Owner ) )
        end)
    end
end)